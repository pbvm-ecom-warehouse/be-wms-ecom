...
async function run() {
  ...
  try {
    ...
    const optionModel = app.get<Model<ItemAttributeOption>>(
      getModelToken(ItemAttributeOption.name),
    );
    ...
    const attributeOptionIds = codes.map((c) => {
      ...
      if (!opt) {
        throw new Error(
          `Thiếu option ${c} trong DB. Hãy chạy pnpm seed:wms trước.`,
        );
      }
    ...
    const createdItem = await stockService.createWarehouseItem(
      dto as any,
      actorId,
    );
